...
import { getAllFiles } from "../../redux/actions/fileFolder.action";
...
const Files = (props) => {
  ...
  const filelist = useSelector((state) => state.fileFolderState.filelist);
  ...
  useEffect(() => {
    dispatch(getAllFiles({ creator: authinfo.currentUser.email }));
            ...
            >
              {contextlist.length > 0 &&
                contextlist.map((item, index) =>
                  listType ? (
                    <RenderEmailbylist item={item} index={index} />
                  ) : (
                    <RenderEmailsbygroup item={item} index={index} />
                  )
                )}
            </ScrollView>
          </View>
        ...
        <View style={{ ...styles.body, height: "70vh" }}>
          {selectedContext !== "" && (
            <View style={styles.inputview}>
              ...
              >
                <ImageBackground
                  source={getIcon(contextlist[selectedContext].file_name)}
                  resizeMode="contian"
                  style={styles.context_avatar}
                />
                <View style={styles.infoGroup}>
